...
class TestService2(unittest.TestCase):
    ...
    def setUp(self):  # s'exécute automatiquement AVANT chaque test
        self.client = app.test_client()  # faux navigateur qui envoie des requêtes sans vrai serveur HTTP
    ...
    def test_describe_succes(self):
        ...
        self.assertEqual(reponse.status_code, 200)  # assertEqual vérifie que les deux valeurs sont égales
    ...
    def test_correlation_succes(self):
        reponse = self.client.post(
            "/stats/correlation",
            json={"x": [1, 2, 3, 4, 5], "y": [2, 4, 6, 8, 10]}  # relation parfaite → r = 1.0
        )
        self.assertEqual(reponse.status_code, 200)
        data = reponse.get_json()
        self.assertAlmostEqual(data["coefficient_pearson"], 1.0, places=2)  # assertAlmostEqual car les flottants ont des imprécisions

    def test_correlation_tailles_differentes(self):
        reponse = self.client.post(
            "/stats/correlation",
            json={"x": [1, 2, 3], "y": [1, 2]}  # tailles différentes → 400 attendu
        )
        self.assertEqual(reponse.status_code, 400)
    ...
    def test_correlation_serie_trop_courte(self):
        reponse = self.client.post(
            "/stats/correlation",
            json={"x": [1], "y": [1]}  # 1 seule valeur → corrélation impossible → 400 attendu
        )
        self.assertEqual(reponse.status_code, 400)
